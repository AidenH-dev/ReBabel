...
export default function ConfigPanelView({
  ...
  const vocabSetOptions = (availableSets || [])
    .filter(
      (set) =>
        set.set_type === 'vocab' && !vocabPool.sets.some((s) => s.id === set.id)
    )
    .map((set) => ({
      value: set.id,
  ...
  const grammarSetOptions = (availableSets || [])
    .filter(
      (set) =>
        set.set_type === 'grammar' &&
        !grammarPool.sets.some((s) => s.id === set.id)
    )
    .map((set) => ({
      value: set.id,
      label: `${set.name} (${set.item_num} items)`,
  ...
  return (
    <div className="md:sticky md:top-0 z-10">
        ...
        <div className="flex flex-col gap-2 bg-white dark:bg-surface-elevated border border-border-subtle rounded-xl p-4 inset-shadow-xs dark:inset-shadow-surface-card">
          ...
          <div className="flex gap-2">
            <Select
              ...
              onChange={(option) =>
                option && onSelectSet(option.value, 'vocabulary')
              }
              placeholder={
                <>
                  <span className="sm:hidden">Select to add sets</span>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {vocabPool.sets.map((set) => (
              ...
              >
                <span>{set.name}</span>
                <button
                  onClick={() => onRemoveSet(set.id, 'vocabulary')}
            ...
            {vocabPool.items.map((item) => (
              ...
              >
                ...
                <button
                  onClick={() => onRemoveItem(item.id, 'vocabulary')}
                  className="hover:opacity-70"
                >
                  <FaTimes className="text-xs" />
                </button>
        ...
        <div className="flex flex-col gap-2 bg-white dark:bg-surface-elevated border border-border-subtle rounded-xl p-4 inset-shadow-xs dark:inset-shadow-surface-card">
          ...
          <div className="flex gap-2">
            <Select
              ...
              onChange={(option) =>
                option && onSelectSet(option.value, 'grammar')
              }
              placeholder={
                <>
                  <span className="sm:hidden">Select to add sets</span>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {grammarPool.sets.map((set) => (
              ...
              >
                <span>{set.name}</span>
                <button
                  onClick={() => onRemoveSet(set.id, 'grammar')}
                  className="hover:opacity-70"
                >
                  <FaTimes className="text-xs" />
                </button>
            ...
            {grammarPool.items.map((item) => (
              ...
              >
                ...
                <button
                  onClick={() => onRemoveItem(item.id, 'grammar')}
                  className="hover:opacity-70"
                >
                  <FaTimes className="text-xs" />
                </button>
          ...
          <div className="flex gap-3">
            <div>
              ...
              <span className="font-medium">
                {poolItems.grammar.length} items
              </span>
            </div>
            <div>
              <span className="text-black/60 dark:text-white/60">
      ...
      {/* Beta Notice */}
      <div className="flex items-center justify-center gap-2 mt-3 px-3 py-2 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/30">
        <span className="text-xs font-semibold text-amber-600 dark:text-amber-400 uppercase tracking-wide">
          Beta
        </span>
        <p className="text-xs text-amber-700 dark:text-amber-300/80">
          This feature may produce inconsistent results. We&apos;re working very
          hard to improve the experience. Your feedback is greatly appreciated!
        </p>
      </div>
    </div>
  );
