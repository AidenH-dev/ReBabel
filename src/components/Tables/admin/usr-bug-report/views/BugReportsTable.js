...
export const BugReportsTable = ({
  ...
  const SortIcon = ({ column }) => {
    ...
    return sortDirection === 'asc' ? (
      <FaArrowUp className="inline ml-1 text-brand-pink" />
    ) : (
      <FaArrowDown className="inline ml-1 text-brand-pink" />
    );
  };
    ...
    <div>
      ...
      {loading && (
        <div className="flex items-center justify-center py-12">
          <FaSpinner className="animate-spin text-2xl text-brand-pink" />
        ...
        <div>
          ...
          <div className="mb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="text-sm text-gray-600 dark:text-gray-400">
              Showing{' '}
              <span className="font-bold text-brand-pink">{startIdx + 1}</span>{' '}
              to{' '}
              <span className="font-bold text-brand-pink">
                {Math.min(endIdx, sortedReports.length)}
              </span>{' '}
              of{' '}
              <span className="font-bold text-brand-pink">
          ...
          {sortedReports.length === 0 ? (
            <div className="bg-surface-card rounded-lg p-8 border border-gray-200 dark:border-gray-800 text-center">
              <p className="text-gray-600 dark:text-gray-400">
                No bug reports found for the selected time range.
              </p>
            </div>
          ) : (
            /* Table */
            <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-800">
              <table className="w-full bg-surface-card">
                <thead className="bg-surface-deep border-b border-gray-200 dark:border-gray-800">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">
                      <button
                        onClick={() => onSort('submitted')}
                        className="flex items-center hover:text-brand-pink transition-colors"
                      >
                        Submitted
