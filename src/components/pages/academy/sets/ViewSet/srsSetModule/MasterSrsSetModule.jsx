...
export default function SRSDashboard({ setId, setData }) {
    ...
    const fetchNewItemsCount = async () => {
      ...
      try {
        ...
        const dayLimitResponse = await fetch(
          `/api/database/v2/srs/set/learn/day-limit/${setId}?limit=5&vocab_or_grammar=${setData.set_type}&as_of=${encodeURIComponent(asOf)}&as_of_timezone=${encodeURIComponent(timezone)}`
        );

        if (!dayLimitResponse.ok) {
          throw new Error(
            `Failed to fetch daily limit: ${dayLimitResponse.statusText}`
          );
        }
        ...
        if (learnNewLeft <= 0) {
          ...
          const response = await fetch(
            `/api/database/v2/srs/set/learn/${setId}?limit=1`
          );

          if (!response.ok) {
            throw new Error(
              `Failed to fetch new items: ${response.statusText}`
            );
          }
          ...
          if (data.success && data.data?.items) {
            ...
            const countResponse = await fetch(
              `/api/database/v2/srs/set/learn/${setId}?limit=${fetchLimit}`
            );
  ...
  const handleLearnNewClick = () => {
    if (learnNewLeftLimit > 0) {
      ...
      router.push(
        `/learn/academy/sets/study/${setId}/srs/learn-new?limit=${limitToUse}`
      );
    }
  };
    ...
    <div className="sm:mb-3 w-full h-fit sm:h-40 group relative p-3 bg-white dark:bg-[#1c2b35] rounded-lg border border-black/5 dark:border-white/10 transition-all shadow-sm flex flex-col">
      ...
      <div className="flex gap-3 flex-1">
        ...
        <div className="grid grid-cols-2 gap-3 sm:w-1/2">
          ...
          {/* Learn New */}
          {(stats.learnNew === 0 || learnNewLeftLimit === 0) &&
          !loadingNewItems ? (
