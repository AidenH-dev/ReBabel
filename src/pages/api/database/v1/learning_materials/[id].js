    ...
    try {
      if (req.method === 'GET') {
        ...
        const { data: learning_material, error: cErr } = await supabaseAdmin
          ...
          .select(
            'id,title,learning_materials,institution,description,study_goal,start_date,end_date,created_at'
          )
          .eq('id', id)
          .eq('owner_id', ownerId)
          .single();
        ...
        const sectionsWithContent = await Promise.all(
          ...
          `
            ...
            if (gErr) {
              req.log.error('grammar.fetch_failed', {
          ...
          `
            ...
            if (vErr) {
              req.log.error('vocabulary.fetch_failed', {
      ...
      if (req.method === 'PATCH') {
        ...
        const update = {
          title: p.title,
          learning_materials:
            p.learningMaterials ?? p.learning_materials ?? null,
