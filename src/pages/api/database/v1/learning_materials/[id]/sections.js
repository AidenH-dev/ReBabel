  ...
  withLogger(async function handler(req, res) {
    ...
    try {
      ...
      if (req.method === 'PUT') {
        ...
        const updatedSectionIds = newSections
          .filter((s) => s.id)
          .map((s) => s.id);
      ...
      async function cleanupOrphanedEntries(ownerId) {
        try {
          ...
          const { data: referencedGrammar, error: gRefErr } =
            await supabaseAdmin.from('section_grammar').select('grammar_id');

          if (gRefErr) {
            req.log.error('cleanup.referenced_grammar_failed', {
              error: gRefErr?.message || String(gRefErr),
            });
            return;
          }

          const referencedGrammarIds = (referencedGrammar || []).map(
            (r) => r.grammar_id
          );
          ...
          if (gAllErr) {
            req.log.error('cleanup.all_grammar_failed', {
              error: gAllErr?.message || String(gAllErr),
            });
            return;
          }
          ...
          if (orphanedGrammarIds.length > 0) {
            req.log.info('cleanup.orphaned_grammar', {
              count: orphanedGrammarIds.length,
            });
            ...
            if (delGrammarErr)
              req.log.error('cleanup.delete_grammar_failed', {
                error: delGrammarErr?.message || String(delGrammarErr),
              });
          }
          ...
          if (vRefErr) {
            req.log.error('cleanup.referenced_vocab_failed', {
              error: vRefErr?.message || String(vRefErr),
            });
            return;
          }

          const referencedVocabIds = (referencedVocab || []).map(
            (r) => r.vocab_id
          );
          ...
          if (vAllErr) {
            req.log.error('cleanup.all_vocab_failed', {
              error: vAllErr?.message || String(vAllErr),
            });
            return;
          }
          ...
          if (orphanedVocabIds.length > 0) {
            req.log.info('cleanup.orphaned_vocab', {
              count: orphanedVocabIds.length,
            });
            ...
            if (delVocabErr)
              req.log.error('cleanup.delete_vocab_failed', {
                error: delVocabErr?.message || String(delVocabErr),
              });
          }
        } catch (error) {
          req.log.error('cleanup.error', {
