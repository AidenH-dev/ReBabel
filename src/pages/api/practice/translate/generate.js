...
export default withApiAuthRequired(
  withLogger(async function handler(req, res) {
    ...
    const grammarFocalPoints = focalPoints.filter(
      (fp) => fp.type === 'grammar'
    );
...
10. In items_used, list the exact id values of every vocabulary and grammar item you used in each sentence (from the id fields in available_vocabulary and available_grammar, or focal_grammar and focal_vocabulary)
            ...
            items: {
              ...
              properties: {
                ...
                items_used: {
                  type: 'object',
                  description:
                    'IDs of pool items actually used in this sentence',
                  properties: {
                    vocabulary_ids: {
                      type: 'array',
                      items: { type: 'string' },
    ...
    try {
      ...
      req.log.error('generation.error', {
