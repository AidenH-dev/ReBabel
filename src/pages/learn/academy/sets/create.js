...
export default function CreateNewSet() {
    ...
    const transformedItems = proposedItems.map((item) => {
      ...
      if (item.type === 'vocabulary') {
        ...
        if (item.example_sentences) {
          ...
          const sentences = item.example_sentences
            .split('\n')
            .filter((s) => s.trim());
  ...
  const handleSubmitAll = async () => {
    ...
    // Count items by type
    const vocabItems = proposedItems.filter(
      (item) => item.type === 'vocabulary'
    ).length;
    const grammarItems = proposedItems.filter(
    ...
    if (vocabItems > 0 && grammarItems > 0) {
      showStatus(
        'A set can only contain vocabulary or grammar items, not both.',
        'error'
      );
      return;
    }
    ...
    try {
      ...
      if (responseData.success) {
        ...
        showStatus(
          'Error: ' + (responseData.error || 'Failed to create set'),
          'error'
        );
      }
    } catch (error) {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 dark:bg-[#141f25]">
      ...
      <main className="ml-auto flex-1 px-4 sm:px-6 py-4 pt-[max(1rem,env(safe-area-inset-top))]">
        <Head>
              ...
              <div className="flex items-center gap-2">
                <div className="flex bg-gray-200 dark:bg-[#0f1a1f] rounded-md p-1">
                  ...
                  <button
                    ...
                    disabled={
                      proposedItems.length > 0 && itemType !== 'grammar'
                    }
