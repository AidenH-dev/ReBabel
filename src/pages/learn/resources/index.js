...
export default function Information() {
    ...
    const articles = useMemo(() => ({
        srs_basics: {
            ...
            content: (
                <div className="space-y-4">
                    <p>
                        Spaced Repetition schedules reviews right before you would forget, strengthening memory with minimal time. A
                        practical starter schedule: <span className="font-medium">10m → 1d → 3d → 1w → 1m → 3m</span>.
                    </p>
                    <ul className="list-disc list-inside space-y-1">
                        <li><span className="font-medium">Keep cards tiny:</span> one idea per card (word meaning, example cloze, or single grammar cue).</li>
                        <li><span className="font-medium">Mix skills:</span> vocab (60%), grammar (25%), kanji (15%) to start; adjust by weaknesses.</li>
                        <li><span className="font-medium">Daily cadence:</span> 10–20 new cards is sustainable while keeping accuracy ≥ 85%.</li>
                    </ul>
                </div>
            ),
        ...
        why_tracks: {
            ...
            content: (
                <div className="space-y-4">
                    <p>
                        The <span className="font-medium">Academy</span> is flexible, material‑driven learning (your textbooks, classes, and custom units). The
                        <span className="font-medium"> Certificate</span> track aligns study to standardized goals (JLPT N5–N1) for test‑day performance.
                    </p>
                    <ul className="list-disc list-inside space-y-1">
                        <li><span className="font-medium">Academy:</span> upload materials, generate drills, track section progress.</li>
                        <li><span className="font-medium">Certificate:</span> level‑tagged decks, mock tests, milestone dates (July/Dec).</li>
                        <li><span className="font-medium">Together:</span> Academy builds breadth; Certificate turns it into exam‑ready skills.</li>
                    </ul>
                </div>
            ),
        ...
        jlpt_overview: {
            ...
            content: (
                <div className="space-y-4">
                    <p>
                        JLPT measures <span className="font-medium">reading and listening</span>. Use level‑tagged SRS decks and graded reading to progress.
                    </p>
    ...
    const WidgetCard = ({ icon, title, subtitle, onOpen, badge, gradient }) => (
        ...
        >
            <div className={`h-1 ${gradient || "bg-gradient-to-r from-gray-200 to-gray-300"}`} />
            ...
            <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
                ...
                <div className="relative w-full max-w-2xl bg-white dark:bg-[#1c2b35] rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                    <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 dark:border-gray-700">
                        <h4 className="text-base font-semibold text-gray-900 dark:text-white">{article.title}</h4>
                    ...
                    <div className="p-5">
                        ...
                        {article.cta && (
                            <div className="mt-5 flex gap-3">
                                ...
                                <button onClick={onClose} className="px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200">
                                    Close
                                </button>
                            </div>
                        )}
    ...
    const ScaledFrame = ({ src, zoom = 0.8, height = 640 }) => (
        <div className="relative w-full overflow-hidden bg-white dark:bg-[#0f171b]" style={{ height }}>
            <div
    ...
    const ExternalTile = ({ name, logoSrc, href, previewSrc, accent, zoom = 0.75, height = 360 }) => (
        <div className="bg-white dark:bg-[#1c2b35] rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2 border-b border-gray-100 dark:border-gray-700 dark:bg-gray-800" >
                ...
                <a href={href} target="_blank" rel="noreferrer" className="text-[#e30a5f] inline-flex items-center gap-1 text-sm hover:opacity-80">
            ...
            {previewSrc ? (
                ...
                <div className="h-64 w-full flex items-center justify-center text-sm text-gray-500 dark:text-gray-400 bg-white dark:bg-[#0f171b]">Preview unavailable</div>
            )}
        </div>
    );
        ...
        <div className="mb-6">
            <div className="bg-white dark:bg-[#1c2b35] rounded-xl shadow-sm p-6">
                <div className="flex items-start justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Information</h1>
                        <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Learn how our platform works, then explore recommended outside tools below.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="flex min-h-screen bg-gray-50 dark:bg-[#141f25]">
            ...
            <main className="ml-auto max-h-screen overflow-y-auto flex-1 px-8 py-6">
                ...
                <section aria-labelledby="external-heading" className="mt-2">
                    <h2 id="external-heading" className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Recommended Outside Resources</h2>

                    {/* Layout: NHK tall on the left; Jisho + JLPT stacked on the right */}
                    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                        {/* Left: NHK tall preview */}
                        <div className="xl:col-span-2 order-1 xl:order-none">
                            <NHKPreviewWindow />
                        </div>

                        {/* Right: two landscape tiles stacked */}
                        <div className="grid grid-cols-1 gap-6 order-2 xl:order-none">
                            <ExternalTile
                                name="Jisho Japanese Dictionary"
                                logoSrc="/logos/jisho.png"
                                href="https://jisho.org/"
                                previewSrc="https://jisho.org/" // may be blocked; still shows logo/CTA
                            ...
                            <ExternalTile
                                ...
                                previewSrc="https://www.jlpt.jp/e/" // may be blocked; still shows logo/CTA
                                accent="linear-gradient(90deg, #fee2e2, #ffe4e6)"
                            />
                        </div>
                    </div>

                    {/* How-to helpers under the tiles */}
                    <div id="nhk-how" className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <section className="bg-white dark:bg-[#1c2b35] rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                            ...
                            <p className="dark:text-white">Skim headlines; choose an interesting topic! NKH Easy is perfect for intermediate to engage realistically with the language.
                                Fill in any blanks with Jisho and see if you can successfully translate the article while reading</p>
                        </section>
                        ...
                        <section className="bg-white dark:bg-[#1c2b35] rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                            ...
                            <ul className="text-sm text-gray-700 dark:text-gray-300 space-y-2">
                                <li><span className="font-medium">Wildcard:</span> * for unknown kana (例: *つもり)</li>
                                <li><span className="font-medium">Kanji pages:</span> check radicals + stroke order</li>
                                <li><span className="font-medium">Examples:</span> short, common sentences → better cards</li>
                            </ul>
                            <div className="mt-3">
                                <a
                                    href="https://jisho.org/"
