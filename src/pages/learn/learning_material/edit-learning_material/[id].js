...
export default function EditLearning_materialPage() {
  ...
  const loadLearning_material = async () => {
    try {
        ...
        const sectionsWithContent = await Promise.all(
          loadedSections.map(async (section) => {
            const contentResp = await fetch(
              `/api/database/v1/sections/${section.id}`
            );
            if (contentResp.ok) {
      ...
      clientLog.error('edit_lm.load_failed', {
  ...
  const deleteGrammarPoint = async (sectionId, grammarId) => {
    try {
      ...
      clientLog.error('edit_lm.delete_grammar_failed', {
        error: e?.message || String(e),
      });
  ...
  const deleteVocabulary = async (sectionId, vocabId) => {
    try {
      ...
      clientLog.error('edit_lm.delete_vocab_failed', {
        error: e?.message || String(e),
      });
      ...
      setSections((prev) =>
        prev.map((s) =>
          s.id === editingSectionId
            ? { ...currentSection, id: editingSectionId }
            : s
        )
      );
      setEditingSectionId(null);
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const infoResp = await fetch(
        `/api/database/v1/learning_materials/${id}`,
        {
          method: 'PATCH',
      ...
      const sectionsResp = await fetch(
        `/api/database/v1/learning_materials/${id}/sections`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
      ...
      clientLog.error('edit_lm.save_failed', {
        error: e?.message || String(e),
      });
  ...
  const handleDeleteLearning_material = async () => {
    if (
      !confirm(
        'Delete this learning material and all its content? This cannot be undone.'
      )
    )
      return;
    try {
      ...
      clientLog.error('edit_lm.delete_failed', {
        error: e?.message || String(e),
      });
...
function Learning_materialInfoTab({
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          >
            ...
            <option value="Online Learning_material">
              Online Learning_material
            </option>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="text-center">
            <p className="text-2xl font-bold text-green-600 dark:text-green-400">
              {sections.reduce(
                (acc, s) => acc + (s.vocabulary?.length || 0),
                0
              )}
            </p>
            <p className="text-xs text-gray-600 dark:text-gray-400">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        {section.grammar?.length || 0} grammar •{' '}
                        {section.vocabulary?.length || 0} vocabulary
                      </p>
                    </div>
                  </div>
                        ...
                        <div className="space-y-2">
                          {section.grammar.map((g) => (
                            ...
                            >
                              ...
                              <button
                                onClick={() =>
                                  actions.deleteGrammarPoint(section.id, g.id)
                                }
                        ...
                        <div className="flex flex-wrap gap-2">
                          {section.vocabulary.map((v) => (
                            ...
                            >
                              ...
                              <button
                                onClick={() =>
                                  actions.deleteVocabulary(section.id, v.id)
                                }
                  ...
                  {currentGrammar.examples.map((example, idx) => (
                    <div key={idx} className="flex gap-2 mb-2">
                      <input
                        ...
                        value={example}
                        onChange={(e) =>
                          actions.updateExample('grammar', idx, e.target.value)
                        }
                  ...
                  {currentVocab.examples.map((example, idx) => (
                    <div key={idx} className="flex gap-2 mb-2">
                      <input
                        ...
                        value={example}
                        onChange={(e) =>
                          actions.updateExample('vocab', idx, e.target.value)
                        }
                        placeholder="Example sentence..."
                        className="flex-1 px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm"
                      />
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                {isEditing
                  ? 'Update this section when ready'
                  : 'Save this section to add it to the learning_material'}
              </div>
              <button
